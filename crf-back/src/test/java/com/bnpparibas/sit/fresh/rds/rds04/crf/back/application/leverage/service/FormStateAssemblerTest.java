 ...
 *
 * <p>The assembler also takes fired validation messages, a locale and the audit stamps. Most tests
 * here have no opinion about those three, so they go through the {@link #assemble} overload and
 * read exactly as they did before; the two nested classes at the end cover them directly.
 */
...
class FormStateAssemblerTest {
    ...
    /**
     * The projection-only shape. Defaults are the "nothing to say" values: no violations, the
     * form's own language, and no analysis behind the call.
     */
    ...
    private static ValidationMessage message(String messageKey, ValidationRule rule,
        ...
        return new ValidationMessage(questionKey, fieldKey, rule, messageKey, Severity.ERROR,
                new LocalizedLabel(textEn, textFr));
    }
    ...
    class InProgress {
        ...
        void pending_maps_to_in_progress_with_the_current_question_marked() {
            ...
            FormState state = assemble(def, Map.of("Q01", "NO"),
        ...
        void flags_are_carried_mid_form() {
            ...
            FormState state = assemble(def, Map.of("Q01", "YES"),
        ...
        void a_computed_answer_is_shown_and_marked_derived() {
            ...
            FormState state = assemble(def, Map.of(),
        ...
        void a_prefilled_answer_is_shown_and_marked_derived() {
            ...
            FormState state = assemble(def, Map.of(), result);
    ...
    class Completed {
        ...
        void terminal_maps_to_completed_and_merges_catalogue_and_branch_flags() {
            FormState state = assemble(preliminary(), Map.of("Q01", "NO"),
        ...
        void an_ecb_form_completes_without_an_outcome() {
            ...
            FormState state = assemble(ecb, Map.of("Q01", "YES"),
    ...
    class Stranded {
        ...
        void a_stranded_walk_raises_rather_than_rendering() {
            ...
            TraversalResult stranded = new TraversalResult(TraversalState.STRANDED, null,
                    Map.of(), Map.of(), Map.of(), null, List.of("Q01"));

            StrandedTraversalException thrown = assertThrows(StrandedTraversalException.class,
                    () -> assemble(def, Map.of(), stranded));
    ...
    class ValidationMessages {
        ...
        void a_fired_message_is_rendered_in_the_requested_language() {
            ValidationMessage mandatory = message("ECB_CHECKLIST_MANDATORY", ValidationRule.MANDATORY,
                    ...
                    "Formulaire BCE - Veuillez repondre aux questions obligatoires.");
            ...
            assertEquals("Formulaire BCE - Veuillez repondre aux questions obligatoires.", view.text());
        }

        /**
         * Most French wordings on the Forms tab are still to be provided, so a French analyst has
         * to read the English rather than an empty alert.
         */
        @Test
        ...
        /** Messages travel on a finished form too, not only while it is being filled. */
        @Test
        void messages_travel_on_a_completed_state() {
            ValidationMessage positive = message("ECB_EBITDA_POSITIVE", ValidationRule.MUST_BE_POSITIVE,
                    "Q-F01", "ebitda", "EBITDA must be positive.", null);

            FormState state = assembler.assemble(ecb(), Map.of("Q01", "YES"),
                    terminal(List.of("Q01"), null, Map.of()), List.of(positive), "EN", FormAudit.NONE);

            assertEquals(FormState.Status.COMPLETED, state.status());
            assertEquals(1, state.validationMessages().size());
        }
    }
