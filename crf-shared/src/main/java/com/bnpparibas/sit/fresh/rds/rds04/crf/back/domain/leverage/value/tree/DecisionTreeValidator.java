...
public final class DecisionTreeValidator {
    ...
    private void validateReachabilityAndCycles(Ctx ctx) {
        ...
        dfs(def.entryQuestion(), ctx, walk, Map.of());

        reportUnreachable(ctx, walk.reached);
        reportDeadEnds(ctx, walk.reached);
     ...
     */
    /** One run's bookkeeping, so the walk methods keep short signatures. */
    private static final class GraphWalk {
        /** Every question reached under ANY set of assumptions. */
        private final Set<String> reached = new LinkedHashSet<>();
        /**
         * Visits already made, keyed by question AND assumptions. Keying by question alone was a
         * bug: with the LBO flag pinned to YES the walk stops at Q-T01's ALL_NO branch, marks
         * Q-S01 seen, and then refuses to re-enter it on the NO path — so Q-T02 and Q-T03, which
         * only that path reaches, were reported UNREACHABLE.
         */
        private final Set<Visit> visited = new HashSet<>();
        /** Current path, in order, so a cycle can be described rather than merely announced. */
        private final Set<String> onStack = new LinkedHashSet<>();

        private boolean firstVisit(String key, Map<String, String> assumed) {
            reached.add(key);
            return visited.add(new Visit(key, assumed)) && visited.size() <= MAX_VISITS;
        }
    }

    private record Visit(String questionKey, Map<String, String> assumed) {
    }

    private void dfs(String key, Ctx ctx, GraphWalk walk, Map<String, String> assumed) {
        if (walk.onStack.contains(key)) {
            reportCycle(ctx, walk, key);
            return;
        }
        if (!walk.firstVisit(key, assumed)) {
            return;
        }
        walk.onStack.add(key);
        Question q = ctx.byKey.get(key);
        if (q != null) {
            walkBranches(q, ctx, walk, assumed);
        }
        walk.onStack.remove(key);
    }

    /** Names the whole loop, because "Q-C02 is in a cycle" does not say which edge to change. */
    private void reportCycle(Ctx ctx, GraphWalk walk, String key) {
        List<String> path = new ArrayList<>(walk.onStack);
        int from = path.indexOf(key);
        String loop = from < 0 ? key : String.join(" -> ", path.subList(from, path.size()));
        ctx.errors.add(Error.question(ctx.ft, key, Aspect.REACHABILITY, "CYCLE",
                "Cycle detected: " + loop + " -> back to " + key));
    }

    private void walkBranches(Question q, Ctx ctx, GraphWalk walk, Map<String, String> assumed) {
        for (Branch branch : nullToEmpty(q.branches())) {
            ...
            followEdge(branch, ctx, walk, merged(assumed, implied));
    ...
    private void followEdge(Branch branch, Ctx ctx, GraphWalk walk, Map<String, String> assumed) {
        if (!branch.isTerminal() && hasText(branch.goTo()) && ctx.byKey.containsKey(branch.goTo())) {
            dfs(branch.goTo(), ctx, walk, assumed);
        }
    }
