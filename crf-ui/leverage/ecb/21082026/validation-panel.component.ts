...
import { AnalysisStatusChangeView, CompletenessBlocker } from './validation-availability.model';
...
export class ValidationPanelComponent {
  ...
  /** Non-null once validated — FormState already carries it, so no payload change. */
  readonly validatedAt = input<string | null>(null);
  readonly validatedBy = input<string | null>(null);

  readonly validated = output<AnalysisStatusChangeView>();
  readonly conflicted = output<void>();

  readonly isValidated = computed(() => this.validatedAt() !== null);
  ...
  readonly reason = computed<string | null>(() => {
    ...
    return this.messageFor(availability.blocker, availability.blockingForm);
  });
