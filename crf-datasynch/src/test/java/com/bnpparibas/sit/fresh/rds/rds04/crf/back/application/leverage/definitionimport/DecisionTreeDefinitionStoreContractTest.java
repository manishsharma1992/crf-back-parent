...
/**
 * The versioning rules every {@link DecisionTreeDefinitionStore} must obey, written once.
 *
...
abstract class DecisionTreeDefinitionStoreContractTest {

    protected abstract DecisionTreeDefinitionStore store();
    ...
    void an_unimported_form_has_version_zero() {
        assertEquals(0, store().currentVersion(LeverageFormType.FED));
    }

    @Test
    void saving_makes_the_definition_current_and_findable() {
        store().save(definition(LeverageFormType.ECB, 1, "Q01"), MARCH, "manish");
        assertEquals(1, store().currentVersion(LeverageFormType.ECB));
        assertTrue(store().findInForce(LeverageFormType.ECB, MARCH.plus(1, ChronoUnit.DAYS)).isPresent());
    }
    ...
    void superseding_closes_the_old_version_without_deleting_it() {
        store().save(definition(LeverageFormType.ECB, 1, "Q01"), MARCH, "manish");
        store().supersede(LeverageFormType.ECB, JULY, "manish");
        store().save(definition(LeverageFormType.ECB, 2, "Q01"), JULY, "manish");

        assertEquals(2, store().currentVersion(LeverageFormType.ECB));
        assertEquals(1, store().findInForce(LeverageFormType.ECB, MARCH.plus(1, ChronoUnit.DAYS))
                .orElseThrow().version(), "March still resolves to the March rules");
        assertEquals(2, store().findInForce(LeverageFormType.ECB, JULY.plus(1, ChronoUnit.DAYS))
                .orElseThrow().version());
    }

    @Test
    void superseding_a_form_with_nothing_open_is_harmless() {
        assertDoesNotThrow(() -> store().supersede(LeverageFormType.PRELIMINARY, JULY, "manish"));
    }
    ...
    void forms_are_versioned_independently() {
        store().save(definition(LeverageFormType.ECB, 1, "Q01"), MARCH, "manish");
        store().save(definition(LeverageFormType.ECB, 2, "Q01"), JULY, "manish");
        store().save(definition(LeverageFormType.FED, 1, "F01"), MARCH, "manish");
        assertEquals(2, store().currentVersion(LeverageFormType.ECB));
        assertEquals(1, store().currentVersion(LeverageFormType.FED));
    }

    @Test
    void nothing_is_in_force_before_its_valid_from() {
        store().save(definition(LeverageFormType.ECB, 1, "Q01"), JULY, "manish");
        assertTrue(store().findInForce(LeverageFormType.ECB, MARCH).isEmpty());
    }
    ...
    void the_stored_definition_round_trips_whole() {
        store().save(definition(LeverageFormType.ECB, 1, "Q01"), MARCH, "manish");
        DecisionTreeDefinition read = store().findInForce(LeverageFormType.ECB, MARCH).orElseThrow();
